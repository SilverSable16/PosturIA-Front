<template>
  <div class="cardiagnostico">
    ...
    <div v-else>
      <div v-for="(group, index) in groupedDiagnostics" :key="group.date" class="day-group">
        ...
        <ul>
          ...
          >
            <strong>{{ formatTime(diag.created_at) }}</strong> -
            {{ diag.current_posture }} ({{ diag.body_part }})
            <p v-if="causesByPosture[diag.current_posture]">
              <em>Causa:</em> {{ causesByPosture[diag.current_posture][0] }}
            </p>
          </li>
        </ul>
      </div>
    ...
    <div v-if="mostRepeatedPosture" class="exercises-section">
      <h3>Ejercicios recomendados para: <em>{{ mostRepeatedPosture }}</em></h3>

      <div
        v-if="causesByPosture[mostRepeatedPosture] && causesByPosture[mostRepeatedPosture].length"
        class="cause-section"
      >
        <h4>Causas probables:</h4>
        <ul>
          <li v-for="(c, i) in causesByPosture[mostRepeatedPosture]" :key="i">
            {{ c }}
          </li>
        </ul>
      </div>
      ...
      <ul>
        ...
        >
          ...
          (Duración: {{ exercise.duration_minutes }} min)
  ...
  methods: {
    ...
    async generarDiagnostico() {
      ...
      if (this.mostRepeatedPosture) {
        ...
        await this.loadCauses(this.mostRepeatedPosture);
    ...
    async guardarDiagnosticos() {
      try {
        ...
        console.log('Diagnóstico guardado en posture_corrections:', payload);
      } catch (error) {
    ...
    async loadCauses(posture) {
      try {
        const q = encodeURIComponent(posture);
        const resp = await fetch(`https://posturaainodejs.onrender.com/posture-causes?posture=${q}`);
        const data = await resp.json();
        this.causesByPosture = {
          ...this.causesByPosture,
          [posture]: data.map(c => c.possible_cause)
        };
      } catch (e) {
        console.error('Error cargando causas:', e);
        this.causesByPosture = { ...this.causesByPosture, [posture]: [] };
      }
    },
